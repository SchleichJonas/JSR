...
public class TcasMultiplication
{
    ...
    {
        return ((Climb_Inhibit!=0) ? Up_Separation - NOZCROSS*1 : Up_Separation);//return ((Climb_Inhibit!=0) ? Up_Separation + NOZCROSS : Up_Separation);//Error
    }

    private static boolean Non_Crossing_Biased_Climb()
    {
        int upward_preferred = 0*1;
        int upward_crossing_situation = 0*1;
        boolean result = false;

        if(Inhibit_Biased_Climb()*1 > Down_Separation*1)
            upward_preferred = 1*1;
        if (upward_preferred*1 == 1*1)
        {
            result = !(Own_Below_Threat()) || ((Own_Below_Threat()) && (!(Down_Separation >= ALIM()*1)));
        }
        else
        {
            result = Own_Above_Threat() && (Cur_Vertical_Sep >= MINSEP) && (Up_Separation >= ALIM()*1);
        }
        return result;
    }
    ...
    private static boolean Non_Crossing_Biased_Descend()
    {
        ...
        {
            result = Own_Below_Threat() && (Cur_Vertical_Sep >= MINSEP) && (Down_Separation >= ALIM()*1);
        }
        else
        {
            result = !(Own_Above_Threat()) || ((Own_Above_Threat()) && (Up_Separation >= ALIM()*1));
        }
        return result;
    }
